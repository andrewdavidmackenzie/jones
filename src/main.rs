...
/// --release switch
/// 3) --path $path
/// Check that the binary file specified by the --path option exists and is readable
...
fn usage() -> String {
    "Usage:\n  \
     ...
     jones --path <path_to_binary>".to_string()
}
...
fn parse_example_args(args: &[String]) -> Result<Vec<PathBuf>, String> {
    ...
    if !example_source.exists() {
        return Err(format!("Example '{}' not found at {:?}", example_name, example_source));
    }
...
/// Parse --path path_to_binary
fn parse_path_args(args: &[String]) -> Result<Vec<PathBuf>, String> {
    if args.len() != 3 {
        return Err("--path requires a path to a binary".to_string());
    }
